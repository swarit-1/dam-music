...
class AudioService {
    ...
    // Monotonic counter to identify the most recent play request and avoid race conditions
    private lastPlayRequestId = 0;
    ...
    async playSound(url: string, loop: boolean = true): Promise<void> {
        try {
            // mark this play request
            const requestId = ++this.lastPlayRequestId;
            ...
            // If another play request was started after this one, discard this loaded sound
            if (requestId !== this.lastPlayRequestId) {
                try {
                    await sound.unloadAsync();
                } catch (e) {
                    // ignore unload errors for stale sounds
                }
                return;
            }
